...
function updateBullet() {
  for (let i = 0; i < missiles.length; i++){
    let top = speedDelta(missiles[i], "top", -8);
    if (top < -10) {
      missiles[i].remove();
      missiles.splice(i, 1);
    }
...
function updateBullet2(playground_h) {
  for (let j = 0; j < alienMissiles.length; j++) {
    let top = speedDelta(alienMissiles[j], "top", +3);
    if (top > playground_h - 10) {
      alienMissiles[j].remove();
  ...
  return () => {
    ...
    for (n of invader) {
      for (m of missiles) {
        if (collision(n, m)) {
          m.remove();
          missiles.filter((e) => e != m)
          n.remove();
          let newInvader = invader.filter((e) => e != n)
          invader = newInvader;
